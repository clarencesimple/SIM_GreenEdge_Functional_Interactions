...
# Treemaps function for actual functional traits

phyloseq_long_treemap_functionaltrait <- function(df, group1, group2, title) {
  ...
  g_treemap <- ggplot(df, aes(area = n_reads,
    ...
    treemapify::geom_treemap_subgroup_text(place = "centre", grow = FALSE, reflow=TRUE, alpha = 0.9, colour =
                                             "white", size=24) +
    scale_fill_manual(values=functionalPalette) + 
    theme(legend.position="none", plot.title = element_text(size = 18, face = "bold"))
  print(g_treemap)
  return(g_treemap)
}
...
phyloseq_long_treemap_clusters <- function(df, group1, group2, group3, title) {

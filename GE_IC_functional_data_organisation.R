...
sample_metadata <- read_excel("./metabarcoding/samples.xlsx") 
...
# Taking only samples involved in GE Ice Camp
taxa <- read_excel("./metabarcoding/asv_cleaned.xlsx") %>%
...
trophic_mode <- read_excel("./assignment_trophic_mode/trophic_mode_updated.xlsx") %>% 
...
########################################
### TAXA FUNCTIONAL TRAIT ASSIGNMENT ###
########################################
taxonomy <- taxa %>%
  filter(trophic_mode=="heterotrophic") %>%
  select(asv_code, domain:species) 

# load functional_trait assignment table (from schneider, schneider majority rules, and manual input)
functional_trait <- read_excel("./assignment_functional_trait/functional_trait_updated.xlsx") %>% 
...
property <- functional_trait

pr2_functional_list <- list()

# This is from manually input allocation
for (taxon_level in  taxon_levels) {
  # Assign trophic mode for PR2 taxa for a given level  
  pr2_functional_list[[taxon_level]] <- property_merge(taxon_level)
  # Remove from PR2 taxa that have been assigned  
  taxonomy <-taxonomy %>% 
    filter(!(species %in% pr2_functional_list[[taxon_level]]$species)) 
  cat(glue::glue("Level {taxon_level} done \n\n"))
}

pr2_functional_trait <- purrr::reduce(pr2_functional_list, bind_rows)

taxa <- taxa %>%
  left_join(pr2_functional_trait %>% select(asv_code,functional_trait,taxon_level),by="asv_code") %>%
  rename(taxon_level_functional=taxon_level) 
...
# add metaPR2 cASVs 
metapr2_cASV <- read_excel("./assignment_biogeography/metapr2 clusters_asv_set_ge2_2022-11-28.xlsx", sheet="H") %>%
...
# add biogeo to ASVs
biogeo<-read.csv("./assignment_biogeography/biogeo_casv.csv") %>% select(-X) %>% select(casv_code,biogeo)
...
#############################
### TAXA SUBSTRATE ORIGIN ###
#############################


#asv substrate origin
#indic <- read_excel("./supplementary_indicspecies_origin.xlsx") %>%
#  select(asv_code,p.value,origin) %>% 
#  filter(p.value<0.05)

ice_specific <- read_xlsx("./GE_IC_indicspecies_RNA.xlsx", sheet="ice") %>%
  mutate(specificity="ice")
ice_specific$asv_code <- substr(ice_specific$asv_code,start = 1, stop = 10)

water_specific <- read_xlsx("./GE_IC_indicspecies_RNA.xlsx", sheet="water") %>%
  mutate(specificity="water")
water_specific$asv_code <- substr(water_specific$asv_code,start = 1, stop = 10)

indic <- rbind(ice_specific,water_specific)

taxa <- left_join(taxa,indic,by="asv_code")
taxa$specificity <- ifelse(is.na(taxa$specificity)=="TRUE", "non_specific", taxa$specificity)
...
taxa$functional_trait <- ifelse (is.na(taxa$functional_trait)==TRUE, "Other_Dinophyceae", taxa$functional_trait)
taxa$functional_trait <- ifelse(taxa$trophic_mode=="heterotrophic" & taxa$subdivision=="Dinoflagellata","Eukaryvore", taxa$functional_trait)
taxa$functional_trait <- ifelse(taxa$trophic_mode=="unassigned_euks","Other_Dinophyceae",taxa$functional_trait)
